...
func WithKubeConfig(kubeConfigFile string) Option {
  return func(ktr *KubernetesTargetRetriever) error {
	...
	if ktr.usePrometheusOperator {
	  o, err := withKubeConfig(client, kubeConfigFile)
	  if err != nil {
		return fmt.Errorf("could create operator target retriever: %w", err)
	  }
...
func WithInClusterConfig() Option {
  return func(ktr *KubernetesTargetRetriever) error {
	...
	if ktr.usePrometheusOperator {
	  o, err := withInClusterConfig(client)
	  if err != nil {
		return fmt.Errorf("could create operator target retriever: %w", err)
	  }
...
type KubernetesTargetRetriever struct {
  ...
  usePrometheusOperator             bool
  operator 							*operatorTargetRetriever
}
...
func NewKubernetesTargetRetriever(scrapeEnabledLabel string, requireScrapeEnabledLabelForNodes, usePrometheusOperator bool, options ...Option) (*KubernetesTargetRetriever, error) {
  ...
  ktr := &KubernetesTargetRetriever{
	...
	usePrometheusOperator: usePrometheusOperator,
  }
...
func (k *KubernetesTargetRetriever) GetTargets() ([]Target, error) {
  ...
  if k.usePrometheusOperator {
	targets = append(targets, k.operator.getTargets()...)
  }
...
func (k *KubernetesTargetRetriever) watchTargets() {
  ...
  if k.usePrometheusOperator {
   	go k.operator.watchOperatorResources()
  }
}
